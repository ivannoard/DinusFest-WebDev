...
import { CardMemo, Hangouts } from './index'
import { useSubscription } from "@apollo/client";
import { SUBS_ALL_FEED_DATA_BY_USER } from '../../graphql/feed'

const Profile = ({ setMenuState }) => {
  const [profileState, setProfileState] = useState('Postingan')
  const [currentView, setCurrentView] = useState();
  const [userId, setUserId] = useState();
  const handleProfile = (state) => {
    setProfileState(state)
  }
  const { data, loading } = useSubscription(SUBS_ALL_FEED_DATA_BY_USER, {
    variables: {
      user_id: userId,
    },
          ...
          <div className="memo-media h-full">
            {
              loading ?
                ...
                :
                <CardMemo setMenuState={setMenuState} data={data?.memolive_feed} />
            }
          </div>);
      case "Badge":
        return setCurrentView(
          <div className="memo-media h-full">
            {
              loading ?
                ...
                :
                <Hangouts data={data?.memolive_feed} />
            }
          </div>);
      default:
        return setCurrentView(
          <div className="memo-media h-full">
            {
              loading ?
                ...
                :
                <CardMemo data={data?.memolive_feed} />
            }
          </div>
        );
    }
  }, [profileState, loading, data, setMenuState]);
  useEffect(() => {
